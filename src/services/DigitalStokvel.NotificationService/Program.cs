...
try
{
    ...
    builder.Services.AddScoped<INotificationService, DigitalStokvel.NotificationService.Services.NotificationService>();
    builder.Services.AddSingleton<INotificationTemplateService, NotificationTemplateService>();

    // Register notification providers (Push, SMS, USSD)
    ...
    builder.Services.AddSingleton<INotificationProvider, UssdNotificationProvider>();
    ...
    Log.Information("Starting Digital Stokvel Notification Service");
    app.Run();
}
catch (Exception ex)
{
